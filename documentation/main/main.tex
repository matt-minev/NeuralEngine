...
\noindent
\begin{center}
\includegraphics[width=\textwidth]{uchimi_logo_black.jpg}
\end{center}

% Keep the title block close to the logo so all elements fit comfortably
\vspace{1.2em}
...
% Overline
{\LARGE\bfseries
ДВАДЕСЕТ И ШЕСТА УЧЕНИЧЕСКА СЕКЦИЯ\\
на Пролетната конференция на СМБ\\
УС$'$26\\
5 -- 8 април 2026 г., Трявна
\par}
...
\subsection{Recent Innovations \& Improvements}

Recent development cycles introduced a major performance and architecture upgrade focused on production readiness of the web applications.

\textbf{GPU Acceleration via Backend Abstraction (Completed)}: The engine now supports explicit device selection with automatic backend resolution (\texttt{cpu}, \texttt{gpu}, \texttt{auto}) and safe fallback behavior. A CuPy execution path was integrated for CUDA-capable systems, while preserving deterministic CPU operation as default fallback. This enables transparent acceleration without breaking existing API usage.

\textbf{Native C++ Core Pipeline (Completed)}: Core numerical execution was reworked around a native C++ extension pathway to reduce Python overhead in hot execution paths. The native runtime is now treated as a first-class backend in the engine architecture, with Python fallback retained for compatibility and debugging.

\textbf{Universal Recognizer CNN Migration (Completed Foundation)}: The Universal Recognizer pipeline was migrated from a dense-only baseline toward a CNN-first training path, with strict preprocessing contract integration and model metadata/version linkage. This establishes the required foundation for high-accuracy retraining and calibrated web inference.
...
\textbf{Engine Optimization (Partially Completed)}: Porting core tensor operations to a GPU-backed backend (CuPy path with CPU fallback) is now implemented. Remaining work includes broader fused-kernel optimization and mixed-precision training (FP16) for higher throughput and lower memory usage.

\textbf{macOS Metal GPU Backend (Planned)}: Add a dedicated Metal-accelerated backend for Apple Silicon systems to provide native GPU training/inference on macOS where CUDA/CuPy is unavailable. This includes kernel parity testing, deterministic fallback rules, and performance benchmarking against native CPU.
...
\textbf{Application Expansion (Partially Completed)}: The Universal Recognizer has begun migration to a CNN-based pipeline with stricter preprocessing contracts. Next steps are full-scale retraining and calibration to satisfy hard accuracy gates on EMNIST ByClass and web-canvas benchmarks. Additional plans include higher-degree polynomial datasets (cubic, quartic), symbolic feature integration (discriminant, coefficient ratios), and domain-shift evaluation on datasets such as USPS and NIST SD-19.
...
\medskip
\noindent\textit{Disclaimer: Selected parts of this document were formatted and linguistically refined with the assistance of artificial intelligence tools. All technical content, claims, and conclusions were reviewed and validated by the author.}
